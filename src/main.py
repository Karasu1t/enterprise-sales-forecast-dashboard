...
def etl_handler(data, context):
    ...
    df['holiday_flug'] = df['holiday_flug'].astype('Int64')
    ...
    try:
        ...
        job_config_query = bigquery.QueryJobConfig(
            query_parameters=[
                bigquery.ArrayQueryParameter("dates", "DATE", df['date'].unique().tolist())
            ]
        )
    ...
    if not df.empty:
        job_config = bigquery.LoadJobConfig(
            ...
            schema=[
                ...
                bigquery.SchemaField("holiday_flug", "INT64"),
            ],
            time_partitioning=bigquery.TimePartitioning(
                type_=bigquery.TimePartitioningType.DAY,
                field="date"
            ),
            source_format=bigquery.SourceFormat.PARQUET if blob.name.endswith('.parquet') else bigquery.SourceFormat.CSV,
        )
