...
Terraform configurations live under `env/dev/` and `modules/`. For the GCS backend, copy `env/dev/backend.hcl.example` to `env/dev/backend.hcl`, set your state bucket, then run `terraform init -backend-config=backend.hcl` from `env/dev/`.

**GitHub Actions secrets:** set `VERTEX_PREDICT_URL` (Vertex predict URL for Streamlit on Cloud Run) and `MODEL_PATH` (GCS URI of training artifacts for deploy). Other GCP values continue to use existing repository secrets.
